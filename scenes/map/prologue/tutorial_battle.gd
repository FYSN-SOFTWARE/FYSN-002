...
func show_tutorial(steps: Array[Dictionary]) -> void:
	...
	# 添加到战斗场景的UI层，确保它在最上层
	battle.battle_ui.add_child(tutorial)
	
	# 确保弹窗在UI层的最前面
	battle.battle_ui.move_child(tutorial, battle.battle_ui.get_child_count() - 1)
	...
	# 打印调试信息
	print("Tutorial displayed with ", len(steps), " pages")
...
func get_basic_tutorial_steps() -> Array[Dictionary]:
	return [
		...
		{
			"texture": preload("res://scenes/map/prologue/22222.jpg"),
			"text": "[b]法力系统：[/b]\n右上角显示当前法力值。\n\n每回合开始时法力值会重置。\n\n合理规划法力使用是胜利的关键！"
		},
		{
			"texture": preload("res://scenes/map/prologue/卡牌.jpg"),
